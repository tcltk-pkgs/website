...
function getSourceForgeIcon() {
    return `<svg fill="currentColor" width="20" height="20" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
...
function showPackageDetail(pkgName, updateHash = true) {
    ...
    const sourcesCards = pkg.sources.map((src, index) => {
        ...
        if (method === 'fossil') {
            ...
            methodClass = 'source-method icon-only';
        } else if (src.url.includes('sourceforge.net')) {
            methodDisplay = getSourceForgeIcon();
            methodClass = 'source-method icon-only';
        } else {
